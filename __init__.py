...
class ContainerChallengeType(BaseChallenge):
    ...
    # Allowlist of fields that may be set via create/update requests
    ALLOWED_FIELDS = {
        'name', 'description', 'category', 'state', 'max_attempts', 'value',
        'image', 'internal_port', 'internal_ports', 'command',
        'container_connection_type', 'container_connection_info',
        'ssh_username', 'ssh_password',
        'memory_limit', 'cpu_limit', 'pids_limit',
        'timeout_minutes', 'max_renewals',
        'flag_mode', 'flag_prefix', 'flag_suffix', 'random_flag_length',
        'container_initial', 'container_minimum', 'container_decay',
        'decay_function',
    }
    ...
    def create(cls, request):
        ...
        # Map field names, exclude UI-only fields, and enforce allowlist
        mapped_data = {}
        for key, value in data.items():
            if key in exclude_fields:
                continue
            mapped_key = field_mapping.get(key, key)
            if mapped_key not in cls.ALLOWED_FIELDS:
                continue
            mapped_data[mapped_key] = value
        
        if mapped_data.get('container_connection_type') == 'ssh':
            ssh_username = data.get('ssh_username', 'ctf')
            ssh_password = data.get('ssh_password', '')
            if ssh_username or ssh_password:
                mapped_data['ssh_username'] = ssh_username
                if ssh_password:
                    mapped_data['ssh_password'] = ContainerConfig.encrypt_value(ssh_password)
                if ssh_username:
                    mapped_data['container_connection_info'] = f"Username: {ssh_username}"
    ...
    def update(cls, challenge, request):
        ...
        for attr, value in data.items():
            ...
            # SECURITY: Enforce field allowlist — reject unknown attributes
            if db_attr not in cls.ALLOWED_FIELDS:
                continue
        ...
        # Handle SSH credentials - build connection_info if SSH is selected
        if challenge.container_connection_type == 'ssh':
            ssh_username = data.get('ssh_username', '')
            ssh_password = data.get('ssh_password', '')
            if ssh_username:
                challenge.ssh_username = ssh_username
            if ssh_password:
                challenge.ssh_password = ContainerConfig.encrypt_value(ssh_password)
            if ssh_username:
                challenge.container_connection_info = f"Username: {ssh_username}"
...
def load(app: Flask):
    ...
    try:
        ...
        if docker_socket.startswith('ssh://'):
            ...
            ssh_dir = _os.path.join(ssh_home, '.ssh')
            _os.makedirs(ssh_dir, mode=0o700, exist_ok=True)
            _os.environ['HOME'] = ssh_home
            
            host_alias = ContainerConfig.get('ssh_host_alias', '')
            ...
            ssh_key_encrypted = ContainerConfig.get('ssh_key_content', '')
            
            if host_alias and ssh_hostname:
                # Restore SSH private key
                key_path = _os.path.join(ssh_dir, f'{host_alias}_key')
                if ssh_key_encrypted:
                    try:
                        ssh_key_plain = ContainerConfig.decrypt_value(ssh_key_encrypted)
                        with open(key_path, 'w') as f:
                            f.write(ssh_key_plain)
                        _os.chmod(key_path, 0o600)
                    except Exception as e:
                        logger.warning(f"Failed to restore SSH key: {e}")
                
                # Restore known_hosts
                known_hosts_path = _os.path.join(ssh_dir, 'known_hosts')
                if ssh_known_hosts:
                    with open(known_hosts_path, 'w') as f:
                        f.write(ssh_known_hosts + '\n')
                    _os.chmod(known_hosts_path, 0o644)
                
                # Restore SSH config
                config_path = _os.path.join(ssh_dir, 'config')
                config_lines = [
                    '# CTFd Docker Plugin - Auto-generated (restored on startup)\n',
                    f'Host {host_alias}\n',
                ...
                config_lines.append(f'    StrictHostKeyChecking yes\n')
                ...
                logger.info(f"Restored SSH config for {host_alias} from database")
            else:
                logger.warning("SSH Docker socket configured but SSH params missing in DB")
...
def _initialize_default_config():
    ...
    defaults = {
        ...
        'max_memory': '512m',
        'max_cpu': '0.5',
        'fame_or_shame': '0',  # 0 = disabled, 1 = enabled
        'teencode_count': '8',  # Number of characters to transform with teencode
    }
